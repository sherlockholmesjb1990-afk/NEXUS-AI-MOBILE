...
app.post("/v1/stream", async (req, res) => {
  ...
  try {
    const { messages = [], cycle_id: cycleId = null } = req.body || {};
    if (cycleId) res.setHeader("X-Nexus-Cycle-ID", String(cycleId).slice(0, 128));
    ...
    for await (const event of stream) {
      if (event.type === "response.output_text.delta") {
        res.write(`data: ${JSON.stringify({ text: event.delta, cycle_id: cycleId })}\n\n`);
      } else if (event.type === "response.completed") {
        res.write(`data: ${JSON.stringify({ done: true, response_id: event.response?.id || null, cycle_id: cycleId })}\n\n`);
      } else if (event.type === "error") {
        res.write(`data: ${JSON.stringify({ error: event.message || "stream_error", cycle_id: cycleId })}\n\n`);
      }
    }
    res.write("data: [DONE]\n\n");
    res.end();
