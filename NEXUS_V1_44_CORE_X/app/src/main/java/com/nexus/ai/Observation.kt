...
data class NexusObservation(
    ...
    fun toJson(): String = JSONObject().apply {
        ...
        put("source", source.take(120)); put("createdAt", createdAt); put("expiresAt", expiresAt); put("cycleId", cycleId)
    }.toString()
}
...
class ObservationStore(private val journal: AgentJournal, private val maxPerSubject: Int = 30) {
    ...
    fun recent(subjectId: String? = null, limit: Int = 12, cycleId: String? = null): List<NexusObservation> =
        journal.observations(subjectId, limit.coerceIn(1, 50), cycleId)
