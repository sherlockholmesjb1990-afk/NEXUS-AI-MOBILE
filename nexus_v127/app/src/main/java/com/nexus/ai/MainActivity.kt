...
class MainActivity : ComponentActivity() {
    ...
    fun NexusApp() {
        ...
        var answer by remember { mutableStateOf("NEXUS V1.24 online.\nAndroid State Observation + Action Adapters + Agent Loop + segurança + verificação + auditoria.") }
                ...
                try {
                    ...
                    if (payload == null && streaming != null && !webSearch) {
                        if (gateway.isNotBlank()) {
                            ...
                            if (gate !is PermissionGate.Result.Allowed) { answer = "🚫 Rede não autorizada."; return@launch }
                        }
                        val messages = core.prepareMessages(text)
                        streaming.streamText(messages).collect { delta -> answer += delta }
        ...
        Column(Modifier.fillMaxSize().padding(16.dp)) {
            ...
            Text("V1.24 • Android State + Action Adapters", style = MaterialTheme.typography.labelLarge)
        ...
        if (showCamera) {
            AlertDialog(onDismissRequest = { showCamera = false }, confirmButton = {}, title = { Text("Visão do NEXUS") }, text = {
                ...
                LaunchedEffect(Unit) { NexusVisionCapture(this@MainActivity, preview, ContextCompat.getMainExecutor(this@MainActivity)).start() }
                Column {
                    AndroidView(factory = { preview }, modifier = Modifier.fillMaxWidth().height(300.dp))
                    Spacer(Modifier.height(8.dp))
                    Button(onClick = {
                        NexusVisionCapture(this@MainActivity, preview, ContextCompat.getMainExecutor(this@MainActivity)).takePhoto(
                            { bmp -> image = bmp; showCamera = false }, { answer = it; showCamera = false }
                        )
                    }) { Text("📸 Capturar") }
                }
            })
